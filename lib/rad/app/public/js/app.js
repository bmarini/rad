$(function(){
  ...
  Rad.Response = Backbone.Model.extend({
  ...
  Rad.ResourceView = Backbone.View.extend({
    ...
    addEndpoint: function(endpoint) {
      var view = new Rad.EndpointView({ model: endpoint, className: "endpoint " + endpoint.get('http_method') });
  ...
  Rad.EndpointView = Backbone.View.extend({
    ...
    initialize: function() {
      _.bindAll(this, "render", "addParams", "addParam", "toggleParams",
        "proxyRequest", "renderResponse");
    },
    ...
    proxyRequest: function() {
      $.post('/proxy', this.$("form").serialize(), this.renderResponse);
      return false;
    },

    renderResponse: function(data) {
      var resp = new Rad.Response(data);
      var view = new Rad.ResponseView({model: resp});
      this.$(".response").html( view.render().el );
    }
  });
  ...
  Rad.ResponseView = Backbone.View.extend({
    template: $("#mustache-response").html(),
